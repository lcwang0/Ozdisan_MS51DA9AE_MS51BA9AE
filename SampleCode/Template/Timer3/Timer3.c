...
#define SyncPin   P01        //P0.1
bit SyncToggle;
#define RamDataLen 10
typedef union
{
		 unsigned char ByteArray[RamDataLen];
		 struct
		 {                                                                                            
					unsigned char MainsToggle;       
					unsigned char MainsBufferCnt;   
					unsigned char MainsCnt;   			 
		 }Data;
}RAM_t;
...
void Timer3_ISR (void) interrupt 16 
{
    ...
    clr_T3CON_TF3;
    P17 = ~P17;
	                RAM.Data.MainsBufferCnt++;
                RAM.Data.MainsCnt++;           

		if(SyncPin==SyncToggle)
		//if(SyncToggle==P01)
		{
			SyncToggle ^= 1;
		}
